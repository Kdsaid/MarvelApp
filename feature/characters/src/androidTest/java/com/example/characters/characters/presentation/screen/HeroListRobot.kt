...
import com.example.characters.presentation.screen.HeroListScreenContent
...
class HeroListRobot(private val composeTestRule: ComposeContentTestRule) {
    ...
    fun onCharacterSearched(searchText: String) {
        onMagnifierTapped()
        composeTestRule.waitUntilExactlyOneExists(hasContentDescription(getString(commonR.string.search_text)))
        composeTestRule.onNodeWithContentDescription(getString(commonR.string.search_text))
            .performClick()
            .performTextInput(searchText)
    }

    fun onMagnifierTapped() {
        composeTestRule.onNodeWithContentDescription(getString(commonR.string.search)).performClick()
    }
...
class HeroListResultRobot(private val composeTestRule: ComposeContentTestRule) {
    ...
    fun searchIsDisplayed() {
        contentDescriptionIsDisplayed(getString(commonR.string.search_text))
    }
